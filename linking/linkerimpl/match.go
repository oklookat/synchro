...
func matchAlbum(origin streaming.ServiceAlbum, albums []streaming.ServiceAlbum) streaming.ServiceAlbum {
	lastWeight := 0.0
	bestIndex := 0
...
func compareAlbums(first, second streaming.ServiceAlbum) (totalWeight float64, exact bool) {
	...
	// but different services may have different order of album artists.
...
func matchTrack(origin streaming.ServiceTrack, tracks []streaming.ServiceTrack) streaming.ServiceTrack {
	lastWeight := 0.0
	bestIndex := 0
...
func compareTracks(first, second streaming.ServiceTrack) (totalWeight float64, exact bool) {
	if shared.IsNil(first, second) {
		return 0, false
	}
