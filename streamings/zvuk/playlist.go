...
func newPlaylist(account streaming.Account, playlist schema.Playlist, client *gozvuk.Client) *Playlist {
	return &Playlist{
...
func (e Playlist) FromAccount() streaming.Account {
	return e.account
}
...
func (e *Playlist) Tracks(ctx context.Context) ([]streaming.ServiceTrack, error) {
	if err := e.cacheTracks(ctx); err != nil {
		return nil, err
	}

	result := []streaming.ServiceTrack{}
...
func (e *Playlist) AddTracks(ctx context.Context, ids []streaming.ServiceEntityID) error {
	if len(ids) == 0 {
		return nil
	}
	...
	// Remove temp track.
	if ids[0] != streaming.ServiceEntityID(_tempPlaylistTrackId) && len(e.playlist.Tracks) > 1 {
		if err := e.RemoveTracks(ctx, []streaming.ServiceEntityID{streaming.ServiceEntityID(_tempPlaylistTrackId)}); err != nil {
			return err
		}
	}
...
func (e *Playlist) RemoveTracks(ctx context.Context, ids []streaming.ServiceEntityID) error {
	var items []schema.PlaylistItem
	...
	if keepTempTrack {
		...
		if err := e.AddTracks(ctx, []streaming.ServiceEntityID{streaming.ServiceEntityID(_tempPlaylistTrackId)}); err != nil {
			return err
		}
	}
...
func (e *Playlist) IsVisible() *bool {
	return &e.playlist.IsPublic
}
