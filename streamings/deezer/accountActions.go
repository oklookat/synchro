...
func (e LikedAlbumsActions) Liked(ctx context.Context) ([]streaming.ServiceEntity, error) {
	albums := []streaming.ServiceEntity{}
	...
	for {
		...
		if len(albumsd.Data) == 0 || albumsd.Next == nil || len(*albumsd.Next) == 0 {
			break
		}
...
func (e *PlaylistActions) MyPlaylists(ctx context.Context) ([]streaming.ServicePlaylist, error) {
	...
	for {
		...
		for i := range resp.Data {
			...
			if resp.Data[i].Creator == nil {
				continue
			}
			if resp.Data[i].Creator.ID != e.client.UserID {
				continue
			}
...
func (e PlaylistActions) Delete(ctx context.Context, entities []streaming.ServiceEntityID) error {
